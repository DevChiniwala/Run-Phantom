...
export const traceloopLlmAdapter: SpanAdapter = {
  ...
  apply(input): AdapterMatch | null {
    ...
    const sys: string[] = [];
    for (const m of list) {
      // Same reason as the ai-sdk adapter: a null element here throws out of
      // ingest and takes the whole batch with it.
 ...
 * `traceloop.span.kind === "tool"`. Run Phantom normalizes the actual tool name
